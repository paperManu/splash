...
class Root
{
  ...
  public:
     ...
     */
    bool addBranchAt(const std::string& path, std::unique_ptr<Branch>&& branch);
     ...
     */
    bool addLeafAt(const std::string& path, std::unique_ptr<Leaf>&& leaf);
     ...
     */
    std::unique_ptr<Branch> cutBranchAt(const std::string& path);
     ...
     */
    std::unique_ptr<Leaf> cutLeafAt(const std::string& path);
  ...
  private:
    ...
    std::list<std::pair<std::string, Leaf::UpdateCallback>> _callbacksToRegister{};
